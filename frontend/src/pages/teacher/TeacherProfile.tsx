...
export default function TeacherProfile() {
  ...
  useEffect(() => {
    if (user && teacherRecord) {
      setForm({
        ...
        directionId: teacherRecord.direction?.id ?? '',
      })
  ...
  const handleSaveProfile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Update teacher-specific fields (bio, direction)
      if (teacherRecord) {
        await teachersApi.updateMe({
          bio: form.bio,
          directionId: form.directionId || undefined,
          avatarUrl: form.avatarUrl || undefined,
        })
        qc.invalidateQueries({ queryKey: ['my-teacher'] })
      }
  ...
  const handleSendCode = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const msg = err?.response?.data?.message
      setPwError(Array.isArray(msg) ? msg.join(', ') : (msg ?? 'Xatolik'))
    } finally {
      setPwSaving(false)
    }
  }
  ...
  const avatarLetter = (form.firstName || user?.firstName || 'T')[0]?.toUpperCase()
    ...
    <div className="max-w-lg mx-auto space-y-5">
      ...
      <div className="bg-white dark:bg-card-dark rounded-2xl shadow-sm p-6">
        ...
        <div className="flex justify-center mb-6">
          <div className="relative">
            ...
            <button type="button" onClick={() => fileRef.current?.click()}
              className="absolute bottom-0 right-0 w-7 h-7 bg-emerald-600 text-white rounded-full flex items-center justify-center hover:bg-emerald-700 transition-colors shadow-md">
              <Camera size={14} />
            </button>
        ...
        <form onSubmit={handleSaveProfile} className="space-y-4">
          ...
          <div>
            ...
            <select className="input-field" value={form.directionId} onChange={(e) => setForm((f) => ({ ...f, directionId: e.target.value }))}>
          ...
          <div>
            ...
            <input className="input-field" placeholder="O'zingiz haqida qisqacha..." value={form.bio} onChange={(e) => setForm((f) => ({ ...f, bio: e.target.value }))} />
          </div>
      ...
      <div className="bg-white dark:bg-card-dark rounded-2xl shadow-sm p-6">
        ...
        <div className="flex items-center gap-2 mb-5">
          {[1, 2].map((s) => (
            <div key={s} className={`flex items-center gap-1.5 text-xs font-medium ${pwStep >= s ? 'text-emerald-600' : 'text-gray-400'}`}>
              <span className={`w-5 h-5 rounded-full flex items-center justify-center text-xs font-bold ${pwStep >= s ? 'bg-emerald-600 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-500'}`}>{s}</span>
